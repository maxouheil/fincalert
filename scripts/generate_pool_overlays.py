...
OUT_DIR = ROOT / 'data' / 'test_results' / 'overlays_top30'
...
def draw_overlays(image_bgr: np.ndarray, detections: List[dict]) -> np.ndarray:
    ...
    for det in detections:
        ...
        color = (255, 0, 0)  # default red (unknown)
        if state == 'blue':
            color = (0, 255, 0)
        elif state == 'green':
            color = (0, 165, 255)
...
def main() -> int:
    ...
    for ft in top:
        ...
        # Download the exact image used by detector (same URL)
        ...
        pools = det.get('all_pools', [])
        overlay = draw_overlays(img, pools)

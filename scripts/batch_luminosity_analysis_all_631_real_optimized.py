#!/usr/bin/env python3
"""
🌙 Batch Analysis - All 631 Fincas (REAL DATA OPTIMIZED)
Version optimisée avec vraies données VIIRS + cache + parallélisation
"""
...
class LuminosityCache:
    """Cache pour les données de luminosité VIIRS"""
    
    def __init__(self, cache_dir):
        self.cache_dir = Path(cache_dir)
        self.cache_dir.mkdir(parents=True, exist_ok=True)
        self.cache_file = self.cache_dir / "luminosity_cache.pkl"
    ...
    def get_cache_key(self, lat, lon, months):
        """Génère une clé de cache pour les coordonnées"""
        return f"{lat:.6f}_{lon:.6f}_{months}"
    
    def get(self, lat, lon, months):
        """Récupère les données du cache"""
        key = self.get_cache_key(lat, lon, months)
        with self.lock:
            return self.cache.get(key)
    
    def set(self, lat, lon, months, data):
...
def analyze_single_finca_with_cache(args):
    """Analyse une seule finca avec cache"""
    finca_id, lat, lon, index, total, cache, months = args
    
    try:
        # Vérifier le cache d'abord
        cache_key = cache.get_cache_key(lat, lon, months)
        cached_data = cache.get(lat, lon, months)
        
        if cached_data:
            # Utiliser les données en cache
            analyzer = NocturnalLuminosityAnalyzer()
            metrics = analyzer.calculate_luminosity_metrics(cached_data)
            ...
            # Récupérer les vraies données VIIRS
            analyzer = NocturnalLuminosityAnalyzer()
            result = analyzer.analyze_finca_luminosity(
                finca_id, lat, lon, months=months, demo=False
            )
            
            # Mettre en cache si succès
            if result['status'] == 'success':
                cache.set(lat, lon, months, result['monthly_data'])
                result['cached'] = False
...
def analyze_all_631_fincas_real_optimized(max_workers=3, months=6):
    """Analyse toutes les 631 fincas avec vraies données VIIRS optimisées"""
    print("🌙 ANALYSE LUMINOSITÉ NOCTURNE - TOUTES LES 631 FINCAS (VRAIES DONNÉES OPTIMISÉES)")
    print("=" * 80)
    print("📡 Utilisation des vraies données VIIRS DNB")
    print(f"⚡ Parallélisation: {max_workers} workers")
    print(f"📅 Période d'analyse: {months} mois (optimisé)")
    print("💾 Cache automatique des données VIIRS")
    ...
    cache = LuminosityCache(cache_dir)
    cache_stats = cache.get_stats()
    print(f"💾 Cache: {cache_stats['total_entries']} entrées ({cache_stats['cache_size_mb']:.1f} MB)")
    ...
    for i, feature in enumerate(features, 1):
        ...
        if all([finca_id, lat, lon]):
            args_list.append((finca_id, lat, lon, i, total_fincas, cache, months))
    ...
    def print_progress(completed, total, success, error, cached):
        ...
        with print_lock:
            ...
            print(f"\r[{bar}] {completed:3d}/{total} ({progress:5.1f}%) | ✅ {success:3d} ❌ {error:3d} 💾 {cached:3d}", end="", flush=True)
    
    # Lancer l'analyse parallélisée
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Soumettre toutes les tâches
        future_to_args = {executor.submit(analyze_single_finca_with_cache, args): args for args in args_list}
        ...
        for future in as_completed(future_to_args):
            ...
            if result_data['success']:
                ...
                if completed_count <= 10:
                    if result['status'] == 'success':
                        cache_status = "💾" if result.get('cached') else "📡"
                        print(f"\n   {cache_status} [{completed_count:3d}] {result['finca_id']} - Score: {result['score']}/5")
            ...
            print_progress(completed_count, total_fincas, success_count, error_count, cached_count)
            
            # Sauvegarde intermédiaire tous les 50 fincas
            if completed_count % 50 == 0:
                intermediate_file = output_dir / f"luminosity_all631_real_intermediate_{timestamp}.json"
                ...
                progress_pct = (completed_count / total_fincas) * 100
                print(f"   📊 Progression: {completed_count}/{total_fincas} ({progress_pct:.1f}%)")
                print(f"   ✅ Succès: {success_count}, ❌ Erreurs: {error_count}, 💾 Cache: {cached_count}")
    ...
    print(f"   💾 Données en cache: {cached_count}/{success_count} ({cached_count/success_count*100:.1f}%)")
    ...
    final_file = output_dir / f"luminosity_all631_real_{timestamp}.json"
    with open(final_file, 'w') as f:
        json.dump(results, f, indent=2)
    
    # Créer un résumé CSV
    csv_file = output_dir / f"luminosity_all631_real_{timestamp}.csv"
    create_csv_summary(results, csv_file)
    
    # Créer un résumé JSON
    summary_file = output_dir / f"luminosity_all631_real_summary_{timestamp}.json"
    create_json_summary(results, total_time, timestamp, summary_file, cache_stats)
    
    print(f"\n💾 FICHIERS GÉNÉRÉS:")
    print(f"   📄 Résultats complets: {final_file}")
    print(f"   📊 Résumé CSV: {csv_file}")
    print(f"   📈 Résumé JSON: {summary_file}")
    print(f"   💾 Cache: {cache.cache_file}")
...
def create_json_summary(results, total_time, timestamp, summary_file, cache_stats):
    ...
    if successful:
        ...
        summary = {
            "timestamp": timestamp,
            "analysis_type": "luminosity_nocturne_all631_real_optimized",
            "total_fincas": len(results),
            "successful_analyses": len(successful),
            "failed_analyses": len(failed),
            "cached_analyses": len(cached),
            "success_rate": len(successful) / len(results),
            "cache_efficiency": len(cached) / len(successful) if successful else 0,
            "processing_time": {
            ...
            "files_generated": [
                f"luminosity_all631_real_{timestamp}.json",
                f"luminosity_all631_real_{timestamp}.csv",
                f"luminosity_all631_real_summary_{timestamp}.json"
            ]
        }
    else:
        summary = {
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Analyse optimisée de luminosité avec vraies données VIIRS")
    parser.add_argument("--workers", type=int, default=3, help="Nombre de workers pour la parallélisation")
    parser.add_argument("--months", type=int, default=6, help="Nombre de mois à analyser (6 ou 12)")
    
    args = parser.parse_args()
    
    try:
        results = analyze_all_631_fincas_real_optimized(max_workers=args.workers, months=args.months)

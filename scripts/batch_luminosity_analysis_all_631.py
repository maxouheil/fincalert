#!/usr/bin/env python3
"""
🌙 Batch Analysis - All 631 Fincas
Analyse la luminosité nocturne de toutes les 631 fincas avec données VIIRS DNB réelles
Monitoring en temps réel avec progression (24/631)
"""
...
def analyze_all_631_fincas():
    """Analyse toutes les 631 fincas avec monitoring en temps réel"""
    print("🌙 ANALYSE LUMINOSITÉ NOCTURNE - TOUTES LES 631 FINCAS")
    print("=" * 80)
    print("📡 Utilisation des données VIIRS DNB réelles (12 mois)")
    ...
    for i, feature in enumerate(features, 1):
        ...
        if not all([finca_id, lat, lon]):
            print(f"⚠️  [{i:3d}/{total_fincas}] Données manquantes pour {finca_id}")
            error_count += 1
            continue
        
        # Affichage de progression avec barre
        progress = (i / total_fincas) * 100
        bar_length = 30
        filled_length = int(bar_length * i // total_fincas)
        bar = '█' * filled_length + '░' * (bar_length - filled_length)
        
        print(f"\r[{bar}] {i:3d}/{total_fincas} ({progress:5.1f}%) - {finca_id}", end="", flush=True)
        ...
        try:
            ...
            if result['status'] == 'success':
                success_count += 1
                # Afficher les détails tous les 50 fincas ou pour les erreurs
                if i % 50 == 0 or i <= 10:
                    print(f"\n   ✅ [{i:3d}] {finca_id} - Score: {result['score']}/5 - {result['reason'][:60]}...")
                ...
                error_count += 1
                print(f"\n   ❌ [{i:3d}] {finca_id} - Erreur: {result['error_message']}")
                
        except Exception as e:
            error_count += 1
            print(f"\n   💥 [{i:3d}] {finca_id} - Exception: {str(e)}")
            results.append({
                'finca_id': finca_id,
                'status': 'error',
                'error_message': str(e)
        ...
        if i % 50 == 0:
            ...
            print(f"   🚀 Vitesse: {i/elapsed_time:.1f} fincas/min")
    ...
    final_file = output_dir / f"luminosity_all631_{timestamp}.json"
    with open(final_file, 'w') as f:
        json.dump(results, f, indent=2)
    
    # Créer un résumé CSV
    csv_file = output_dir / f"luminosity_all631_{timestamp}.csv"
    create_csv_summary(results, csv_file)
    
    # Créer un résumé JSON
    summary_file = output_dir / f"luminosity_all631_summary_{timestamp}.json"
...
def create_json_summary(results, total_time, timestamp, summary_file):
    ...
    if successful:
        ...
        summary = {
            "timestamp": timestamp,
            "analysis_type": "luminosity_nocturne_all631",
            ...
            "files_generated": [
                f"luminosity_all631_{timestamp}.json",
                f"luminosity_all631_{timestamp}.csv",
                f"luminosity_all631_summary_{timestamp}.json"
            ]
        }
    else:
        summary = {
...
if __name__ == "__main__":
    try:
        results = analyze_all_631_fincas()
